function objmap_2021(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)

%objmap_2021.m -

%   Input Variables Definitions
% variable: A string that will be part of saved names of output figures and
% logfile. Example '2021Temp'
...
load('sf2021_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2021_input_profiles.nc','lat');
% lon = ncread('sf2021_input_profiles.nc','lon');
% CT = ncread('sf2021_input_profiles.nc','CT');
% SA = ncread('sf2021_input_profiles.nc','SA');
% station = ncread('sf2021_input_profiles.nc','stationID');
% dist_plot = ncread('sf2021_input_profiles.nc','distance');
% time = ncread('sf2021_input_profiles.nc','time');
...
whichcasts = [1,2,3,4,5,7,8,10,11,12]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2021 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2021T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2021.T.objmap = objmap;
grids2021.T.errmap = errmap;
grids2021.T.residuals = residuals_mat;
grids2021.T.xg = XG;
grids2021.T.yg = YG;

%save input data into structure
grids2021.T.data = TempData;
grids2021.T.xdata = xx2;
grids2021.T.ydata = depth2;
grids2021.T.latdata = lat;
grids2021.T.londata = lon;
grids2021.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2021.T.data_raw = CT';
grids2021.T.lat_raw = lat_raw;
grids2021.T.lon_raw = lon_raw;
grids2021.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2021S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2021.S.objmap = objmap;
grids2021.S.errmap = errmap;
grids2021.S.residuals = residuals_mat;
grids2021.S.xg = XG;
grids2021.S.yg = YG;
%save input data into structure
grids2021.S.data = SalData;
grids2021.S.xdata = xx2;
grids2021.S.ydata = depth2;
grids2021.S.latdata = lat;
grids2021.S.londata = lon;
grids2021.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2021.S.data_raw = SA';
grids2021.S.lat_raw = lat_raw;
grids2021.S.lon_raw = lon_raw;
grids2021.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2021.T.objmap,grids2021.S.objmap,grids2021.T.xg,grids2021.T.yg,grids2021.T.data,grids2021.T.xdata,grids2021.T.ydata,lat(1));

%save objective map outputs into structure
grids2021.T.objmap = objmapT_corr;
grids2021.T.residuals = residualsT_corr;
...
grids2021.T.errmap_stdgrid = interp2(grids2021.T.xg,grids2021.T.yg,grids2021.T.errmap,Xq,Yq);
grids2021.S.errmap_stdgrid = interp2(grids2021.S.xg,grids2021.S.yg,grids2021.S.errmap,Xq,Yq);

grids2021.T.objmap_stdgrid = interp2(grids2021.T.xg,grids2021.T.yg,grids2021.T.objmap,Xq,Yq);
grids2021.S.objmap_stdgrid = interp2(grids2021.S.xg,grids2021.S.yg,grids2021.S.objmap,Xq,Yq);

grids2021.T.Xq_stdgrid = Xq;
grids2021.S.Xq_stdgrid = Xq;

grids2021.T.Yq_stdgrid = Yq;
grids2021.S.Yq_stdgrid = Yq;

grids2021.notes = notes;
% 
...
grids2021.parameteres = parameters;

grids2021.raw_profile_stationID = station;
grids2021.raw_profile_date = time;
grids2021.lat_raw = lat_raw;
grids2021.lon_raw = lon_raw;

save('grids2021.mat','grids2021')
disp('grids2021 saved')
end
