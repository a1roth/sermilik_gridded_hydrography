function objmap_2023(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)

%objmap_2023.m -
%Only ship-based CTDS. A separate function is for the xctds. 
...
load('sf2023_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2023_input_profiles.nc','lat');
% lon = ncread('sf2023_input_profiles.nc','lon');
% CT = ncread('sf2023_input_profiles.nc','CT');
% SA = ncread('sf2023_input_profiles.nc','SA');
% station = ncread('sf2023_input_profiles.nc','stationID');
% dist_plot = ncread('sf2023_input_profiles.nc','distance');
% time = ncread('sf2023_input_profiles.nc','time');
...
whichcasts = [3,4,5,6,7,8,9,10]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2023 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2023T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2023.T.objmap = objmap;
grids2023.T.errmap = errmap;
grids2023.T.residuals = residuals_mat;
grids2023.T.xg = XG;
grids2023.T.yg = YG;

%save input data into structure
grids2023.T.data = TempData;
grids2023.T.xdata = xx2;
grids2023.T.ydata = depth2;
grids2023.T.latdata = lat;
grids2023.T.londata = lon;
grids2023.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2023.T.data_raw = CT';
grids2023.T.lat_raw = lat_raw;
grids2023.T.lon_raw = lon_raw;
grids2023.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2023S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2023.S.objmap = objmap;
grids2023.S.errmap = errmap;
grids2023.S.residuals = residuals_mat;
grids2023.S.xg = XG;
grids2023.S.yg = YG;
%save input data into structure
grids2023.S.data = SalData;
grids2023.S.xdata = xx2;
grids2023.S.ydata = depth2;
grids2023.S.latdata = lat;
grids2023.S.londata = lon;
grids2023.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2023.S.data_raw = SA';
grids2023.S.lat_raw = lat_raw;
grids2023.S.lon_raw = lon_raw;
grids2023.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2023.T.objmap,grids2023.S.objmap,grids2023.T.xg,grids2023.T.yg,grids2023.T.data,grids2023.T.xdata,grids2023.T.ydata,lat(1));

%save objective map outputs into structure
grids2023.T.objmap = objmapT_corr;
grids2023.T.residuals = residualsT_corr;
...
grids2023.T.errmap_stdgrid = interp2(grids2023.T.xg,grids2023.T.yg,grids2023.T.errmap,Xq,Yq);
grids2023.S.errmap_stdgrid = interp2(grids2023.S.xg,grids2023.S.yg,grids2023.S.errmap,Xq,Yq);

grids2023.T.objmap_stdgrid = interp2(grids2023.T.xg,grids2023.T.yg,grids2023.T.objmap,Xq,Yq);
grids2023.S.objmap_stdgrid = interp2(grids2023.S.xg,grids2023.S.yg,grids2023.S.objmap,Xq,Yq);

grids2023.T.Xq_stdgrid = Xq;
grids2023.S.Xq_stdgrid = Xq;

grids2023.T.Yq_stdgrid = Yq;
grids2023.S.Yq_stdgrid = Yq;

grids2023.notes = notes;
% 
...
grids2023.parameteres = parameters;

grids2023.raw_profile_stationID = station;
grids2023.raw_profile_date = time;
grids2023.lat_raw = lat_raw;
grids2023.lon_raw = lon_raw;

save('grids2023.mat','grids2023')
disp('grids2023 saved')
