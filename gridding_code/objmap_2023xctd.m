function objmap_2023xctd(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)

%objmap_2023xctd.m -
%Only XCTDs from 2023xctd in Helheim Fjord. 
...
load('sf2023xctd_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2023xctd_input_profiles.nc','lat');
% lon = ncread('sf2023xctd_input_profiles.nc','lon');
% CT = ncread('sf2023xctd_input_profiles.nc','CT');
% SA = ncread('sf2023xctd_input_profiles.nc','SA');
% station = ncread('sf2023xctd_input_profiles.nc','stationID');
% dist_plot = ncread('sf2023xctd_input_profiles.nc','distance');
% time = ncread('sf2023xctd_input_profiles.nc','time');
...
whichcasts = [1,2,3,4,5,9]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2023xctd TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2023xctdT',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2023xctd.T.objmap = objmap;
grids2023xctd.T.errmap = errmap;
grids2023xctd.T.residuals = residuals_mat;
grids2023xctd.T.xg = XG;
grids2023xctd.T.yg = YG;

%save input data into structure
grids2023xctd.T.data = TempData;
grids2023xctd.T.xdata = xx2;
grids2023xctd.T.ydata = depth2;
grids2023xctd.T.latdata = lat;
grids2023xctd.T.londata = lon;
grids2023xctd.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2023xctd.T.data_raw = CT';
grids2023xctd.T.lat_raw = lat_raw;
grids2023xctd.T.lon_raw = lon_raw;
grids2023xctd.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2023xctdS',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2023xctd.S.objmap = objmap;
grids2023xctd.S.errmap = errmap;
grids2023xctd.S.residuals = residuals_mat;
grids2023xctd.S.xg = XG;
grids2023xctd.S.yg = YG;
%save input data into structure
grids2023xctd.S.data = SalData;
grids2023xctd.S.xdata = xx2;
grids2023xctd.S.ydata = depth2;
grids2023xctd.S.latdata = lat;
grids2023xctd.S.londata = lon;
grids2023xctd.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2023xctd.S.data_raw = SA';
grids2023xctd.S.lat_raw = lat_raw;
grids2023xctd.S.lon_raw = lon_raw;
grids2023xctd.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2023xctd.T.objmap,grids2023xctd.S.objmap,grids2023xctd.T.xg,grids2023xctd.T.yg,grids2023xctd.T.data,grids2023xctd.T.xdata,grids2023xctd.T.ydata,lat(1));

%save objective map outputs into structure
grids2023xctd.T.objmap = objmapT_corr;
grids2023xctd.T.residuals = residualsT_corr;
...
grids2023xctd.T.errmap_stdgrid = interp2(grids2023xctd.T.xg,grids2023xctd.T.yg,grids2023xctd.T.errmap,Xq,Yq);
grids2023xctd.S.errmap_stdgrid = interp2(grids2023xctd.S.xg,grids2023xctd.S.yg,grids2023xctd.S.errmap,Xq,Yq);

grids2023xctd.T.objmap_stdgrid = interp2(grids2023xctd.T.xg,grids2023xctd.T.yg,grids2023xctd.T.objmap,Xq,Yq);
grids2023xctd.S.objmap_stdgrid = interp2(grids2023xctd.S.xg,grids2023xctd.S.yg,grids2023xctd.S.objmap,Xq,Yq);

grids2023xctd.T.Xq_stdgrid = Xq;
grids2023xctd.S.Xq_stdgrid = Xq;

grids2023xctd.T.Yq_stdgrid = Yq;
grids2023xctd.S.Yq_stdgrid = Yq;

grids2023xctd.notes = notes;
% 
...
grids2023xctd.parameteres = parameters;

grids2023xctd.raw_profile_stationID = station;
grids2023xctd.raw_profile_date = time;
grids2023xctd.lat_raw = lat_raw;
grids2023xctd.lon_raw = lon_raw;

save('grids2023xctd.mat','grids2023xctd')
disp('grids2023xctd saved')
