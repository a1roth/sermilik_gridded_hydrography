function objmap_2015(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)
...
load('sf2015_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2015_input_profiles.nc','lat');
% lon = ncread('sf2015_input_profiles.nc','lon');
% CT = ncread('sf2015_input_profiles.nc','CT');
% SA = ncread('sf2015_input_profiles.nc','SA');
% station = ncread('sf2015_input_profiles.nc','stationID');
% dist_plot = ncread('sf2015_input_profiles.nc','distance');
% time = ncread('sf2015_input_profiles.nc','time');
...
whichcasts = [1, 2,8,9,10,11,12,14, 15, 16, 17]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2015 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2015T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2015.T.objmap = objmap;
grids2015.T.errmap = errmap;
grids2015.T.residuals = residuals_mat;
grids2015.T.xg = XG;
grids2015.T.yg = YG;

%save input data into structure
grids2015.T.data = TempData;
grids2015.T.xdata = xx2;
grids2015.T.ydata = depth2;
grids2015.T.latdata = lat;
grids2015.T.londata = lon;
grids2015.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2015.T.data_raw = CT';
grids2015.T.lat_raw = lat_raw;
grids2015.T.lon_raw = lon_raw;
grids2015.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2015S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2015.S.objmap = objmap;
grids2015.S.errmap = errmap;
grids2015.S.residuals = residuals_mat;
grids2015.S.xg = XG;
grids2015.S.yg = YG;
%save input data into structure
grids2015.S.data = SalData;
grids2015.S.xdata = xx2;
grids2015.S.ydata = depth2;
grids2015.S.latdata = lat;
grids2015.S.londata = lon;
grids2015.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2015.S.data_raw = SA';
grids2015.S.lat_raw = lat_raw;
grids2015.S.lon_raw = lon_raw;
grids2015.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2015.T.objmap,grids2015.S.objmap,grids2015.T.xg,grids2015.T.yg,grids2015.T.data,grids2015.T.xdata,grids2015.T.ydata,lat(1));

%save objective map outputs into structure
grids2015.T.objmap = objmapT_corr;
grids2015.T.residuals = residualsT_corr;
...
grids2015.T.errmap_stdgrid = interp2(grids2015.T.xg,grids2015.T.yg,grids2015.T.errmap,Xq,Yq);
grids2015.S.errmap_stdgrid = interp2(grids2015.S.xg,grids2015.S.yg,grids2015.S.errmap,Xq,Yq);

grids2015.T.objmap_stdgrid = interp2(grids2015.T.xg,grids2015.T.yg,grids2015.T.objmap,Xq,Yq);
grids2015.S.objmap_stdgrid = interp2(grids2015.S.xg,grids2015.S.yg,grids2015.S.objmap,Xq,Yq);

grids2015.T.Xq_stdgrid = Xq;
grids2015.S.Xq_stdgrid = Xq;

grids2015.T.Yq_stdgrid = Yq;
grids2015.S.Yq_stdgrid = Yq;

grids2015.notes = notes;
% 
...
grids2015.parameteres = parameters;

grids2015.raw_profile_stationID = station;
grids2015.raw_profile_date = time;
grids2015.lat_raw = lat_raw;
grids2015.lon_raw = lon_raw;

save('grids2015.mat','grids2015')
disp('grids2015 saved')
