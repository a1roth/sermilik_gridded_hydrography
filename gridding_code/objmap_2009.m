function objmap_2009(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres, xstart, xend, ystart,yend,notes)
...
% background: This is the background field that you wish to use. The time mean
...
load('sf2009_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2009_input_profiles.nc','lat');
% lon = ncread('sf2009_input_profiles.nc','lon');
% CT = ncread('sf2009_input_profiles.nc','CT');
% SA = ncread('sf2009_input_profiles.nc','SA');
% station = ncread('sf2009_input_profiles.nc','stationID');
% dist_plot = ncread('sf2009_input_profiles.nc','distance');
% time = ncread('sf2009_input_profiles.nc','time');
...
whichcasts = [1, 2, 3, 6, 7, 8, 9]; %which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR.
...
for i = 1:length(whichcasts)
    c = whichcasts(i);
    nn = find(isnan(CT(c,4:end)),1,'first'); %have to start at index 3 because there are nans.
    endindex(i) = nn + 2;
...
%% CREATE OBJECTIVE MAP FOR 2009 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2009T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);



%% save objective map outputs into structure
grids2009.T.objmap = objmap;
grids2009.T.errmap = errmap;
grids2009.T.residuals = residuals_mat;
grids2009.T.xg = XG;
grids2009.T.yg = YG;

%save input data into structure
grids2009.T.data = TempData;
grids2009.T.xdata = xx2;
grids2009.T.ydata = depth2;
grids2009.T.latdata = lat;
grids2009.T.londata = lon;
grids2009.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2009.T.data_raw = CT';
grids2009.T.lat_raw = lat_raw;
grids2009.T.lon_raw = lon_raw;
grids2009.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2009S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2009.S.objmap = objmap;
grids2009.S.errmap = errmap;
grids2009.S.residuals = residuals_mat;
grids2009.S.xg = XG;
grids2009.S.yg = YG;
%save input data into structure
grids2009.S.data = SalData;
grids2009.S.xdata = xx2;
grids2009.S.ydata = depth2;
grids2009.S.latdata = lat;
grids2009.S.londata = lon;
grids2009.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2009.S.data_raw = SA';
grids2009.S.lat_raw = lat_raw;
grids2009.S.lon_raw = lon_raw;
grids2009.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';



%% APPLY DATA CORRECTIONS
%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2009.T.objmap,grids2009.S.objmap,grids2009.T.xg,grids2009.T.yg,grids2009.T.data,grids2009.T.xdata,grids2009.T.ydata,lat(1));

%save objective map outputs into structure
grids2009.T.objmap = objmapT_corr;
grids2009.T.residuals = residualsT_corr;
...
grids2009.T.errmap_stdgrid = interp2(grids2009.T.xg,grids2009.T.yg,grids2009.T.errmap,Xq,Yq);
grids2009.S.errmap_stdgrid = interp2(grids2009.S.xg,grids2009.S.yg,grids2009.S.errmap,Xq,Yq);

grids2009.T.objmap_stdgrid = interp2(grids2009.T.xg,grids2009.T.yg,grids2009.T.objmap,Xq,Yq);
grids2009.S.objmap_stdgrid = interp2(grids2009.S.xg,grids2009.S.yg,grids2009.S.objmap,Xq,Yq);

grids2009.T.Xq_stdgrid = Xq;
grids2009.S.Xq_stdgrid = Xq;

grids2009.T.Yq_stdgrid = Yq;
grids2009.S.Yq_stdgrid = Yq;

grids2009.notes = notes;
% 
...
grids2009.parameteres = parameters;

grids2009.raw_profile_stationID = station;
grids2009.raw_profile_date = time;
grids2009.lat_raw = lat_raw;
grids2009.lon_raw = lon_raw;

save('grids2009.mat','grids2009')
disp('grids2009 saved')
