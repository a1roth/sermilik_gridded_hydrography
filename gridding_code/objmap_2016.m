function objmap_2016(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)
...
load('sf2016_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2016_input_profiles.nc','lat');
% lon = ncread('sf2016_input_profiles.nc','lon');
% CT = ncread('sf2016_input_profiles.nc','CT');
% SA = ncread('sf2016_input_profiles.nc','SA');
% station = ncread('sf2016_input_profiles.nc','stationID');
% dist_plot = ncread('sf2016_input_profiles.nc','distance');
% time = ncread('sf2016_input_profiles.nc','time');
...
whichcasts = [1, 2, 3, 4,7,8,10,11]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2016 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2016T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2016.T.objmap = objmap;
grids2016.T.errmap = errmap;
grids2016.T.residuals = residuals_mat;
grids2016.T.xg = XG;
grids2016.T.yg = YG;

%save input data into structure
grids2016.T.data = TempData;
grids2016.T.xdata = xx2;
grids2016.T.ydata = depth2;
grids2016.T.latdata = lat;
grids2016.T.londata = lon;
grids2016.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2016.T.data_raw = CT';
grids2016.T.lat_raw = lat_raw;
grids2016.T.lon_raw = lon_raw;
grids2016.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2016S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2016.S.objmap = objmap;
grids2016.S.errmap = errmap;
grids2016.S.residuals = residuals_mat;
grids2016.S.xg = XG;
grids2016.S.yg = YG;
%save input data into structure
grids2016.S.data = SalData;
grids2016.S.xdata = xx2;
grids2016.S.ydata = depth2;
grids2016.S.latdata = lat;
grids2016.S.londata = lon;
grids2016.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2016.S.data_raw = SA';
grids2016.S.lat_raw = lat_raw;
grids2016.S.lon_raw = lon_raw;
grids2016.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2016.T.objmap,grids2016.S.objmap,grids2016.T.xg,grids2016.T.yg,grids2016.T.data,grids2016.T.xdata,grids2016.T.ydata,lat(1));

%save objective map outputs into structure
grids2016.T.objmap = objmapT_corr;
grids2016.T.residuals = residualsT_corr;
...
grids2016.T.errmap_stdgrid = interp2(grids2016.T.xg,grids2016.T.yg,grids2016.T.errmap,Xq,Yq);
grids2016.S.errmap_stdgrid = interp2(grids2016.S.xg,grids2016.S.yg,grids2016.S.errmap,Xq,Yq);

grids2016.T.objmap_stdgrid = interp2(grids2016.T.xg,grids2016.T.yg,grids2016.T.objmap,Xq,Yq);
grids2016.S.objmap_stdgrid = interp2(grids2016.S.xg,grids2016.S.yg,grids2016.S.objmap,Xq,Yq);

grids2016.T.Xq_stdgrid = Xq;
grids2016.S.Xq_stdgrid = Xq;

grids2016.T.Yq_stdgrid = Yq;
grids2016.S.Yq_stdgrid = Yq;

grids2016.notes = notes;
% 
...
grids2016.parameteres = parameters;

grids2016.raw_profile_stationID = station;
grids2016.raw_profile_date = time;
grids2016.lat_raw = lat_raw;
grids2016.lon_raw = lon_raw;

save('grids2016.mat','grids2016')
disp('grids2016 saved')
