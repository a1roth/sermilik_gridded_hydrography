function objmap_2013(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)
...
load('sf2013_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2013_input_profiles.nc','lat');
% lon = ncread('sf2013_input_profiles.nc','lon');
% CT = ncread('sf2013_input_profiles.nc','CT');
% SA = ncread('sf2013_input_profiles.nc','SA');
% station = ncread('sf2013_input_profiles.nc','stationID');
% dist_plot = ncread('sf2013_input_profiles.nc','distance');
% time = ncread('sf2013_input_profiles.nc','time');
...
whichcasts = [1, 2, 3,8,9,10,11,12,13,14]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2013 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2013T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2013.T.objmap = objmap;
grids2013.T.errmap = errmap;
grids2013.T.residuals = residuals_mat;
grids2013.T.xg = XG;
grids2013.T.yg = YG;

%save input data into structure
grids2013.T.data = TempData;
grids2013.T.xdata = xx2;
grids2013.T.ydata = depth2;
grids2013.T.latdata = lat;
grids2013.T.londata = lon;
grids2013.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2013.T.data_raw = CT';
grids2013.T.lat_raw = lat_raw;
grids2013.T.lon_raw = lon_raw;
grids2013.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2013S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2013.S.objmap = objmap;
grids2013.S.errmap = errmap;
grids2013.S.residuals = residuals_mat;
grids2013.S.xg = XG;
grids2013.S.yg = YG;
%save input data into structure
grids2013.S.data = SalData;
grids2013.S.xdata = xx2;
grids2013.S.ydata = depth2;
grids2013.S.latdata = lat;
grids2013.S.londata = lon;
grids2013.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2013.S.data_raw = SA';
grids2013.S.lat_raw = lat_raw;
grids2013.S.lon_raw = lon_raw;
grids2013.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2013.T.objmap,grids2013.S.objmap,grids2013.T.xg,grids2013.T.yg,grids2013.T.data,grids2013.T.xdata,grids2013.T.ydata,lat(1));

%save objective map outputs into structure
grids2013.T.objmap = objmapT_corr;
grids2013.T.residuals = residualsT_corr;
...
grids2013.T.errmap_stdgrid = interp2(grids2013.T.xg,grids2013.T.yg,grids2013.T.errmap,Xq,Yq);
grids2013.S.errmap_stdgrid = interp2(grids2013.S.xg,grids2013.S.yg,grids2013.S.errmap,Xq,Yq);

grids2013.T.objmap_stdgrid = interp2(grids2013.T.xg,grids2013.T.yg,grids2013.T.objmap,Xq,Yq);
grids2013.S.objmap_stdgrid = interp2(grids2013.S.xg,grids2013.S.yg,grids2013.S.objmap,Xq,Yq);

grids2013.T.Xq_stdgrid = Xq;
grids2013.S.Xq_stdgrid = Xq;

grids2013.T.Yq_stdgrid = Yq;
grids2013.S.Yq_stdgrid = Yq;

grids2013.notes = notes;
% 
...
grids2013.parameteres = parameters;

grids2013.raw_profile_stationID = station;
grids2013.raw_profile_date = time;
grids2013.lat_raw = lat_raw;
grids2013.lon_raw = lon_raw;

save('grids2013.mat','grids2013')
disp('grids2013 saved')
