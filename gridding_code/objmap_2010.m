function obmap_2010(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)

%obmap_2010.m -
%Only XCTDs from this year 
...
load('sf2010_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2010_input_profiles.nc','lat');
% lon = ncread('sf2010_input_profiles.nc','lon');
% CT = ncread('sf2010_input_profiles.nc','CT');
% SA = ncread('sf2010_input_profiles.nc','SA');
% station = ncread('sf2010_input_profiles.nc','stationID');
% dist_plot = ncread('sf2010_input_profiles.nc','distance');
% time = ncread('sf2010_input_profiles.nc','time');
...
whichcasts = [1,6,8,10]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR.
...
CT_plot_smooth = smoothdata(CT_plot_bottomfill,2,'movmean',10,'omitnan'); %includenan or omitnan
SA_plot_smooth = smoothdata(SA_plot_bottomfill,2,'movmean',10,'omitnan');
...
%% CREATE OBJECTIVE MAP FOR 2010 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2010T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2010.T.objmap = objmap;
grids2010.T.errmap = errmap;
grids2010.T.residuals = residuals_mat;
grids2010.T.xg = XG;
grids2010.T.yg = YG;

%save input data into structure
grids2010.T.data = TempData;
grids2010.T.xdata = xx2;
grids2010.T.ydata = depth2;
grids2010.T.latdata = lat;
grids2010.T.londata = lon;
grids2010.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2010.T.data_raw = CT';
grids2010.T.lat_raw = lat_raw;
grids2010.T.lon_raw = lon_raw;
grids2010.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2010S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2010.S.objmap = objmap;
grids2010.S.errmap = errmap;
grids2010.S.residuals = residuals_mat;
grids2010.S.xg = XG;
grids2010.S.yg = YG;
%save input data into structure
grids2010.S.data = SalData;
grids2010.S.xdata = xx2;
grids2010.S.ydata = depth2;
grids2010.S.latdata = lat;
grids2010.S.londata = lon;
grids2010.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2010.S.data_raw = SA';
grids2010.S.lat_raw = lat_raw;
grids2010.S.lon_raw = lon_raw;
grids2010.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2010.T.objmap,grids2010.S.objmap,grids2010.T.xg,grids2010.T.yg,grids2010.T.data,grids2010.T.xdata,grids2010.T.ydata,lat(1));

%save objective map outputs into structure
grids2010.T.objmap = objmapT_corr;
grids2010.T.residuals = residualsT_corr;
...
grids2010.T.errmap_stdgrid = interp2(grids2010.T.xg,grids2010.T.yg,grids2010.T.errmap,Xq,Yq);
grids2010.S.errmap_stdgrid = interp2(grids2010.S.xg,grids2010.S.yg,grids2010.S.errmap,Xq,Yq);

grids2010.T.objmap_stdgrid = interp2(grids2010.T.xg,grids2010.T.yg,grids2010.T.objmap,Xq,Yq);
grids2010.S.objmap_stdgrid = interp2(grids2010.S.xg,grids2010.S.yg,grids2010.S.objmap,Xq,Yq);

grids2010.T.Xq_stdgrid = Xq;
grids2010.S.Xq_stdgrid = Xq;

grids2010.T.Yq_stdgrid = Yq;
grids2010.S.Yq_stdgrid = Yq;

grids2010.notes = notes;
% 
...
grids2010.parameteres = parameters;

grids2010.raw_profile_stationID = station;
grids2010.raw_profile_date = time;
grids2010.lat_raw = lat_raw;
grids2010.lon_raw = lon_raw;

save('grids2010.mat','grids2010')
disp('grids2010 saved')
