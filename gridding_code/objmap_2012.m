function objmap_2012(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)
...
load('sf2012_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2012_input_profiles.nc','lat');
% lon = ncread('sf2012_input_profiles.nc','lon');
% CT = ncread('sf2012_input_profiles.nc','CT');
% SA = ncread('sf2012_input_profiles.nc','SA');
% station = ncread('sf2012_input_profiles.nc','stationID');
% dist_plot = ncread('sf2012_input_profiles.nc','distance');
% time = ncread('sf2012_input_profiles.nc','time');
...
whichcasts = [1, 2, 3, 4, 5,6,8,9,10,11,13,14,15]; %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2012 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2012T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2012.T.objmap = objmap;
grids2012.T.errmap = errmap;
grids2012.T.residuals = residuals_mat;
grids2012.T.xg = XG;
grids2012.T.yg = YG;

%save input data into structure
grids2012.T.data = TempData;
grids2012.T.xdata = xx2;
grids2012.T.ydata = depth2;
grids2012.T.latdata = lat;
grids2012.T.londata = lon;
grids2012.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2012.T.data_raw = CT';
grids2012.T.lat_raw = lat_raw;
grids2012.T.lon_raw = lon_raw;
grids2012.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2012S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2012.S.objmap = objmap;
grids2012.S.errmap = errmap;
grids2012.S.residuals = residuals_mat;
grids2012.S.xg = XG;
grids2012.S.yg = YG;
%save input data into structure
grids2012.S.data = SalData;
grids2012.S.xdata = xx2;
grids2012.S.ydata = depth2;
grids2012.S.latdata = lat;
grids2012.S.londata = lon;
grids2012.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2012.S.data_raw = SA';
grids2012.S.lat_raw = lat_raw;
grids2012.S.lon_raw = lon_raw;
grids2012.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2012.T.objmap,grids2012.S.objmap,grids2012.T.xg,grids2012.T.yg,grids2012.T.data,grids2012.T.xdata,grids2012.T.ydata,lat(1));

%save objective map outputs into structure
grids2012.T.objmap = objmapT_corr;
grids2012.T.residuals = residualsT_corr;
...
grids2012.T.errmap_stdgrid = interp2(grids2012.T.xg,grids2012.T.yg,grids2012.T.errmap,Xq,Yq);
grids2012.S.errmap_stdgrid = interp2(grids2012.S.xg,grids2012.S.yg,grids2012.S.errmap,Xq,Yq);

grids2012.T.objmap_stdgrid = interp2(grids2012.T.xg,grids2012.T.yg,grids2012.T.objmap,Xq,Yq);
grids2012.S.objmap_stdgrid = interp2(grids2012.S.xg,grids2012.S.yg,grids2012.S.objmap,Xq,Yq);

grids2012.T.Xq_stdgrid = Xq;
grids2012.S.Xq_stdgrid = Xq;

grids2012.T.Yq_stdgrid = Yq;
grids2012.S.Yq_stdgrid = Yq;

grids2012.notes = notes;
% 
...
grids2012.parameteres = parameters;

grids2012.raw_profile_stationID = station;
grids2012.raw_profile_date = time;
grids2012.lat_raw = lat_raw;
grids2012.lon_raw = lon_raw;

save('grids2012.mat','grids2012')
disp('grids2012 saved')
end
