function obmap_2011_res(xcor1,ycor1,a1,xcor2,ycor2,a2,noise,xres,yres,xstart,xend,ystart,yend,notes)
...
load('sf2011_input_profiles.mat')
...
%or as netcdf
% lat = ncread('sf2011_input_profiles.nc','lat');
% lon = ncread('sf2011_input_profiles.nc','lon');
% CT = ncread('sf2011_input_profiles.nc','CT');
% SA = ncread('sf2011_input_profiles.nc','SA');
% station = ncread('sf2011_input_profiles.nc','stationID');
% dist_plot = ncread('sf2011_input_profiles.nc','distance');
% time = ncread('sf2011_input_profiles.nc','time');
...
whichcasts = [1, 2, 3, 4, 7,8,9,10,11,12];  %%which rows are deep enough to be extended. CHANGE THIS AS NEEDED FOR YOUR SPECIFIC SITE/YEAR. 
...
%% CREATE OBJECTIVE MAP FOR 2011 TEMPERATURE

%Make objective map
[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2011T',xx2,depth2,TempData,xg,yg,background_CT,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% save objective map outputs into structure
grids2011.T.objmap = objmap;
grids2011.T.errmap = errmap;
grids2011.T.residuals = residuals_mat;
grids2011.T.xg = XG;
grids2011.T.yg = YG;

%save input data into structure
grids2011.T.data = TempData;
grids2011.T.xdata = xx2;
grids2011.T.ydata = depth2;
grids2011.T.latdata = lat;
grids2011.T.londata = lon;
grids2011.T.dist = dist_plot;

%Want some way to save non-bottomfilled points to data CT_plot
grids2011.T.data_raw = CT';
grids2011.T.lat_raw = lat_raw;
grids2011.T.lon_raw = lon_raw;
grids2011.T.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';

%% RUN OBJECTIVE MAP FOR SALINITY

[objmap,errmap,residuals_mat,XG,YG] = objmap_from_profiles('2011S',xx2,depth2,SalData,xg,yg,background_SA,xback,yback,xcor1,ycor1,a1,xcor2,ycor2,a2,noise);

%% Save salinity outputs into structure
grids2011.S.objmap = objmap;
grids2011.S.errmap = errmap;
grids2011.S.residuals = residuals_mat;
grids2011.S.xg = XG;
grids2011.S.yg = YG;
%save input data into structure
grids2011.S.data = SalData;
grids2011.S.xdata = xx2;
grids2011.S.ydata = depth2;
grids2011.S.latdata = lat;
grids2011.S.londata = lon;
grids2011.S.dist = dist_plot;


%Want some way to save non-bottomfilled points to data SA_plot
grids2011.S.data_raw = SA';
grids2011.S.lat_raw = lat_raw;
grids2011.S.lon_raw = lon_raw;
grids2011.S.info = 'data_raw is original profiles with no bottom-filled points. Only actually measured points. data is profiles with bottoms-filled in and what goes into the mapping algorith. objmap is the objective map of the profiles to the specified grid. errmap is the calculated uncertainty of the grid. residuals is the difference between input data (data) and objmap at the points of the input data.';


%% APPLY DATA CORRECTIONS

%CORREECT FOR TEMERATURE VALUES BELOW FREEZING POINT AND RESAVE
[objmapT_corr,residualsT_corr] = correctFreezing(grids2011.T.objmap,grids2011.S.objmap,grids2011.T.xg,grids2011.T.yg,grids2011.T.data,grids2011.T.xdata,grids2011.T.ydata,lat(1));

%save objective map outputs into structure
grids2011.T.objmap = objmapT_corr;
grids2011.T.residuals = residualsT_corr;
...
grids2011.T.errmap_stdgrid = interp2(grids2011.T.xg,grids2011.T.yg,grids2011.T.errmap,Xq,Yq);
grids2011.S.errmap_stdgrid = interp2(grids2011.S.xg,grids2011.S.yg,grids2011.S.errmap,Xq,Yq);

grids2011.T.objmap_stdgrid = interp2(grids2011.T.xg,grids2011.T.yg,grids2011.T.objmap,Xq,Yq);
grids2011.S.objmap_stdgrid = interp2(grids2011.S.xg,grids2011.S.yg,grids2011.S.objmap,Xq,Yq);

grids2011.T.Xq_stdgrid = Xq;
grids2011.S.Xq_stdgrid = Xq;

grids2011.T.Yq_stdgrid = Yq;
grids2011.S.Yq_stdgrid = Yq;

grids2011.notes = notes;
% 
...
grids2011.parameteres = parameters;

grids2011.raw_profile_stationID = station;
grids2011.raw_profile_date = time;
grids2011.lat_raw = lat_raw;
grids2011.lon_raw = lon_raw;

save('grids2011.mat','grids2011')
disp('grids2011 saved')
